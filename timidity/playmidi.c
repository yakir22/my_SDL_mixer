...
#define MIDI_USER_EVENT 0xF000F001
enum MidiEvents
{
	MIDI_NOTE_ON = MIDI_USER_EVENT + 1,
	MIDI_NOTE_OFF = 0xF000F001,
};


#define NotifyMidiEvent(evt,cnl)	SDL_Event user_event;\
									user_event.type = SDL_USEREVENT;\
									user_event.user.code = MIDI_USER_EVENT;\
									user_event.user.data1 = (void*)evt;\
									user_event.user.data2 = (void*)cnl;\
	...
	while (song->current_sample < end_sample) {
		...
		while (song->current_event->time <= song->current_sample) {
			...
			case ME_NOTEON:
				...
				{
					NotifyMidiEvent(MIDI_NOTE_OFF, song->current_event->channel);
					note_off(song);
				}
				else
				{
					NotifyMidiEvent(MIDI_NOTE_ON, song->current_event->channel);
					note_on(song);
				}
				break;
